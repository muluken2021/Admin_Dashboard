  ...
  return (
    <>
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button className="flex items-center gap-2 rounded border border-stroke bg-white px-4 py-2 font-medium text-black hover:shadow-1 dark:border-strokedark dark:bg-meta-4 dark:text-white">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-4 rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark">
        <input
          ...
          className="w-full max-w-md rounded border border-stroke bg-gray py-2 px-4 focus:border-primary outline-none dark:border-strokedark dark:bg-meta-4 text-black dark:text-white"
          value={searchTerm}
          ...
          <table className="w-full table-auto text-left">
            <thead>
              <tr className="bg-gray-2 dark:bg-meta-4">
                <th className="px-4 py-4 font-medium text-black dark:text-white">Order ID</th>
            ...
            <tbody>
              {filteredOrders.map((order) => (
                <tr key={order.orderId} className="border-b border-stroke dark:border-strokedark">
                  <td className="px-4 py-5 font-medium text-black dark:text-white">{order.orderId}</td>
                  ...
                  <td className="px-4 py-5 font-medium text-black dark:text-white">${order.amount.toFixed(2)}</td>
                  <td className="px-4 py-5 text-center">
                    <span className={`inline-flex rounded-full px-3 py-1 text-sm font-medium ${getStatusStyles(order.status)}`}>
                      {order.status}
                    </span>
                  </td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} className="max-w-[600px] p-8">
        {selectedOrder && (
          <div className="no-scrollbar max-h-[85vh] overflow-y-auto">
            ...
            <div className="border-y border-stroke py-4 dark:border-strokedark mb-6">
              <h4 className="font-bold text-black dark:text-white mb-2">Customer Info</h4>
              <p className="text-sm">{selectedOrder.customer.name}</p>
              <p className="text-sm">{selectedOrder.customer.email}</p>
            </div>

            <div className="mb-6">
              <h4 className="font-bold text-black dark:text-white mb-3">Items</h4>
              <div className="flex justify-between items-center bg-gray-2 p-3 rounded dark:bg-meta-4">
                <p className="text-black dark:text-white">{selectedOrder.product}</p>
                <p className="font-bold text-black dark:text-white">${selectedOrder.amount.toFixed(2)}</p>
              </div>
            </div>

            <div className="flex flex-col gap-3">
              <h4 className="font-bold text-black dark:text-white">Update Status</h4>
